...
def loadImgSizes():
    return np.load('../data/img_size.npy', allow_pickle=True)

def loadEncBit(path, num):
    dcallY = np.load(path + '/YDC/' + f'dcallY_{num}.npy', allow_pickle=True)
    acallY = np.load(path + '/YAC/' + f'acallY_{num}.npy', allow_pickle=True)
    dcallCb = np.load(path + '/CbDC/' + f'dcallCb_{num}.npy', allow_pickle=True)
    acallCb = np.load(path + '/CbAC/' + f'acallCb_{num}.npy', allow_pickle=True)
    dcallCr = np.load(path + '/CrDC/' + f'dcallCr_{num}.npy', allow_pickle=True)
    acallCr = np.load(path + '/CrAC/' + f'acallCr_{num}.npy', allow_pickle=True)
    return dcallY, acallY, dcallCb, acallCb, dcallCr, acallCr


def loadEncBits(ConPlain='N'):
